  ...
  def save_people_to_file
    ...
    @people.each do |person|
      if person.is_a?(Student)
        people_store << { id: person.id, age: person.age, name: person.name, parent_permission: person.parent_permission, classroom: person.classroom, className: person.class }
      end
      next unless person.is_a?(Teacher)

      people_store << { id: person.id, name: person.name, age: person.age,  specialization: person.specialization, className: person.class }
    end
